...
echo "<p align ='center'>\n";
echo "<b>Ergebnisse und Tabelle der Begegnungen vom ".$tag.". Spieltag der Saison ".$saison.", ".datum($datum)."</b>\n";
echo "<div class='auswahl' align='center'><a href='index.php?content=archiv&other=1'>anderen Spieltag w&auml;hlen...</a></div><br>\n";
echo "</p>\n";
...
$befehl="select tag_heim from spieltag where tag_saison='$saison' and tag_index='$tag'";
$antwort=mysql_query($befehl);
while($data=mysql_fetch_array($antwort))
{
 $heim=$data['tag_heim'];
 $unterbefehl ="select tag_gast, tag_erg from spieltag where tag_saison='$saison'";
...
// schließende Klammer für MANNSCHAFTEN UND ERGEBNISSE AUSLESEN UND DARSTELLEN
}
...
// MySql-TABELLE MIT ALLEN TABELLENEINTRÃGEN ALLER TEAMS ERZEUGEN
$teambefehl ="select distinct tag_heim from spieltag where tag_heim<>'x'";
$teambefehl.=" and tag_saison=$saison";
...
$befehl.=" and tag_index<='$tag' order by tag_index";
$antwort=mysql_query($befehl);
while($data=mysql_fetch_array($antwort))
{
 $datum=$data['tag_datum'];
...
<form action="index.php?content=archiv&other=1" method="post" name="Tag">
...
<tr>
  ...
  <input type="submit" name="Tag" value="w&auml;hlen">
    ...
    // SAISON NICHT GEWÄHLT
      ...
      $antwort=mysql_query("select distinct tag_saison from spieltag where tag_liga is null order by tag_saison desc");
      ?>
<form action="index.php?content=archiv&other=1" method="post" name="Saison">
...
$befehl="select max(tag_datum) from spieltag where tag_datum<'$auswahl' and tag_liga is null";
$antwort=mysql_query($befehl);
$data=mysql_fetch_array($antwort);
...
// schlieÃende Klammer fÃ¼r MANNSCHAFTEN UND ERGEBNISSE AUSLESEN UND DARSTELLEN
}
...
while($data=mysql_fetch_array($antwort))
{
 ...
 // von momentan gewÃ¤hltem Datum Gegner und Ergebnis auslesen und darstellen
 $unterbefehl ="select tag_heim, tag_gast, tag_erg from spieltag where tag_datum='$datum'";
 $unterantwort=mysql_query($unterbefehl);
